...
const CLOUD_STATE_TYPE = "life_command_state";
const CLOUD_STATE_TITLE = "Life Command State";

let supabaseClient = null;
let cloudStateId = null;
...
async function setupSupabase() {
  ...
  if (!window.supabase) {
    try {
      ...
      try {
        ...
        console.warn("Supabase library did not load. App will keep running with local storage.", jsdelivrError);
        return null;
      }
    }
  }
  ...
  if (!window.supabase) {
    console.warn("Supabase is still unavailable after loading CDN.");
    return null;
  }
...
function applyCloudPayload(payload) {
  ...
  rollingLog = Array.isArray(payload.rollingLog) ? payload.rollingLog : [];
}
...
async function loadCloudState() {
  ...
  try {
    ...
    if (!data || data.length === 0) {
      console.log("No cloud state yet. Creating one from this device.");
      await saveCloudStateNow();
      return true;
    }

    cloudStateId = data[0].id;
    applyCloudPayload(data[0].data);
    saveAllLocalOnly();
    console.log("Loaded Life Command state from Supabase");
    return true;
  } catch (error) {
...
async function saveCloudStateNow() {
  const client = await setupSupabase();
  if (!client || isLoadingCloudState) return;
  ...
  try {
    if (!cloudStateId) {
      ...
      if (error) {
        console.error("Could not check existing cloud state:", error);
        return;
      }

      if (data && data.length > 0) cloudStateId = data[0].id;
    }
    ...
    if (cloudStateId) {
      const { error } = await client
        .from("life_entries")
        .update({
          title: CLOUD_STATE_TITLE,
          data: payload
        })
        .eq("id", cloudStateId);

      if (error) {
        console.error("Could not update cloud state:", error);
        return;
      }
    } else {
      const { data, error } = await client
        ...
        .insert({
          ...
          title: CLOUD_STATE_TITLE,
          content: "Synced Life Command dashboard state",
          status: "active",
          data: payload
        })
        .select("id")
      ...
      if (error) {
        console.error("Could not create cloud state:", error);
        return;
      }

      cloudStateId = data?.id || null;
    }

    console.log("Saved Life Command state to Supabase");
  } catch (error) {
...
const LAUNCH_LIBRARY_URL = "https://ll.thespacedevs.com/2.2.0/launch/upcoming/?limit=50&mode=detailed";
...
function renderProjects() {
  ...
  projects.forEach(project => {
    ...
    card.innerHTML = `
      ...
      <button class="delete-project-button" type="button" onclick="event.stopPropagation(); deleteProject('${project.id}')">Delete</button>
    `;
...
async function deleteProject(projectId) {
  ...
  const confirmed = window.confirm(`Delete "${project.name}"? This removes its todos, events, files, notes, and GitHub connection.`);
  ...
  calendar = calendar.filter(event => event.projectId !== projectId);
...
function filterUpcomingLaunches(launches) {
  const now = new Date();
  const weekFromNow = new Date(now.getTime() + 7 * 24 * 60 * 60 * 1000);

  return (launches || [])
    .filter(launch => {
      const launchTime = launch.net || launch.window_start || launch.windowStart || launch.date;
    ...
    .sort((a, b) => {
      const aDate = new Date(a.net || a.window_start || a.windowStart || a.date);
      const bDate = new Date(b.net || b.window_start || b.windowStart || b.date);
      return aDate - bDate;
    });
}
...
async function loadUpcomingLaunches() {
  ...
  try {
    const response = await fetch(`${LAUNCH_LIBRARY_URL}&_=${Date.now()}`, { cache: "no-store" });
    if (!response.ok) throw new Error("Launch fetch failed");

    const data = await response.json();
    spaceLaunches = filterUpcomingLaunches(data.results || []);
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  setInterval(loadSpaceDashboard, 900000);
